...
def is_it_there(
    ...
    if move == "True":
        ...
        for ID in ids_remain:
            assert ID in id_in_from + id_in_to, f"{ID} should not be in {test_from} repo but it is"
    else:
        for ID in id_in_from:
...
class TestMoveEmbargoArgs(unittest.TestCase):     
    ...
    def test_main_move(self):
        ...
        """
        move = "True"
        is_it_there(self.embargo_hours, self.now_time_embargo, self.ids_remain, self.ids_moved, self.test_from, self.test_to, move=move)
